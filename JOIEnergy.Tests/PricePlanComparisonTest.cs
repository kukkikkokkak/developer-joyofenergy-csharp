...
namespace JOIEnergy.Tests
{
    public class PricePlanComparisonTest
    {
        ...
        private PricePlanComparatorController controllerWithNoMultipliers;
        private PricePlanComparatorController controllerWithMultipliers;
        ...
        public PricePlanComparisonTest()
        {
            ...
            var pricePlansWithMultipliers = new List<PricePlan>() {
                new PricePlan() { PlanName = PRICE_PLAN_1_ID, UnitRate = 10, PeakTimeMultiplier = new List<PeakTimeMultiplier>()
                {
                ...
                new PricePlan() { PlanName = PRICE_PLAN_2_ID, UnitRate = 1, PeakTimeMultiplier = NoMultipliers() },
                new PricePlan() { PlanName = PRICE_PLAN_3_ID, UnitRate = 5, PeakTimeMultiplier = new List<PeakTimeMultiplier>(){
                    new PeakTimeMultiplier() {
                        DayOfWeek = DayOfWeek.Sunday,
                        Multiplier = 5m
                    }
            ...
            var pricePlanServiceWithMultipliers = new PricePlanService(pricePlansWithMultipliers, meterReadingService);
            ...
            controllerWithNoMultipliers = new PricePlanComparatorController(pricePlanService, accountService);
            controllerWithMultipliers = new PricePlanComparatorController(pricePlanServiceWithMultipliers, accountService);
        }

        [Fact]
        public void ShouldCalculateCostForMeterReadingsForEveryPricePlanWithNoMultipliers()
        {
            ...
            Dictionary<string, object> result = controllerWithNoMultipliers.CalculatedCostForEachPricePlan(SMART_METER_ID).Value as Dictionary<string, object>;
            //Console.WriteLine("CalculatedCostForEachPricePlan: result =>" + JsonConvert.SerializeObject(result));
        ...
        [Fact]
        public void ShouldCalculateCostForMeterReadingsForEveryPricePlanWithMultipliers() 
        {
            //var electricityReading = new ElectricityReading() { Time = DateTime.Now.AddHours(-1), Reading = 10.0m };
            var otherReading = new ElectricityReading() { Time = DateTime.Now, Reading = 10.0m };
            meterReadingService.StoreReadings(SMART_METER_ID, new List<ElectricityReading>() {otherReading });

            Dictionary<string, object> result = controllerWithMultipliers.CalculatedCostForEachPricePlanWithMultipliers(SMART_METER_ID).Value as Dictionary<string, object>;
            Assert.NotNull(result);
            Assert.Equal(PRICE_PLAN_1_ID, result[PricePlanComparatorController.PRICE_PLAN_ID_KEY]);
            var expected = new Dictionary<string, decimal>() {
                { PRICE_PLAN_1_ID, 1000m },
                { PRICE_PLAN_2_ID, 10m },
                { PRICE_PLAN_3_ID, 250m },
            };
            Assert.Equal(expected, result[PricePlanComparatorController.PRICE_PLAN_COMPARISONS_KEY]);
        }
        ...
        public void ShouldRecommendCheapestPricePlansNoLimitForMeterUsage()
        {
            ...
            object result = controllerWithNoMultipliers.RecommendCheapestPricePlans(SMART_METER_ID, null).Value;
        ...
        public void ShouldRecommendLimitedCheapestPricePlansForMeterUsage()
        {
            ...
            object result = controllerWithNoMultipliers.RecommendCheapestPricePlans(SMART_METER_ID, 2).Value;
        ...
        public void ShouldRecommendCheapestPricePlansMoreThanLimitAvailableForMeterUsage()
        {
            ...
            object result = controllerWithNoMultipliers.RecommendCheapestPricePlans(SMART_METER_ID, 5).Value;
        ...
        {
            Assert.Equal(404, controllerWithNoMultipliers.CalculatedCostForEachPricePlan("not-found").StatusCode);
        }
